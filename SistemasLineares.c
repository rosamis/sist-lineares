...
//Copia todos os elementos de um SL
SistLinear_t * copiaSL(SistLinear_t *SL)
{
  unsigned int tam = SL->n;
  SistLinear_t *cp = alocaSistLinear(tam);
  for (unsigned int i=0; i<tam; ++i) {
    for (unsigned int j=0; j<tam; ++j)  {
      cp->A[i*tam+j] = SL->A[i*tam+j];
    }
  }
...
//Inverte as linhas de um SL
void inverteLinhas(SistLinear_t *SL, unsigned int i, unsigned int ipivo)
{
  unsigned int tam = SL->n;
  real_t aux;
  for (unsigned int k=0; k<tam; ++k){
    aux = SL->A[i*tam+k];
    SL->A[i*tam+k] = SL->A[ipivo*tam+k];
    SL->A[ipivo*tam+k] = aux;
  }
  aux = SL->b[i];
  SL->b[i] = SL->b[ipivo];
  SL->b[ipivo] = aux;
...
unsigned int encontraMaxPivo(SistLinear_t *SL, unsigned int i)
{
  unsigned int n = SL->n;
  real_t max = SL->A[i];
  unsigned int imax = i;
  for (unsigned int j = i; j < n ; ++j) {
    if (max < SL->A[i*n+j] && SL->A[i*n+j] != 0){
      imax = j; 
      max = SL->A[imax];
    }
  }
...
void inicializaSistLinear (SistLinear_t *SL, tipoSistLinear_t tipo, real_t coef_max)
{
  ...
  // ERRO! RAND_MAX muito grande, não cabe em float e sempre dá 0
